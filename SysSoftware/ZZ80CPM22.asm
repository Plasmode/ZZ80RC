...
;*
; memory organization:
;  0x0-0xFFF is physical page 0 that contains bootstrap code.  It is write protected by MMU
;  0x1000-0xFFFF is page 1 to page F of 64K memory
;  0x10000-0x3BFFF contains CP/M22 system files, drive A:
;  0x3C000-0x3CFFF is logical page 0 that will replace physical page 0 when MMU is enabled
;  0x3D000-0x3FFFF is CP/M 2.2 executable
;  0x40000-0x7FFFF is RAM disk (drive B:)
; 8/5/18 Fork from cpm22all_ramdisk-in-0x80000_3.5meg_RAMdisk
...
ccp 	equ 3400h+bias		;base of ccp  (0xDC00)
bdos 	equ ccp+806h		;base of bdos (0xE406)
bios 	equ ccp+1600h		;base of bios (0xF200)
cdisk 	equ 0004h		;current disk number 0=a,...,15=p
iobyte 	equ 0003h		;intel i/o byte
;;; Z280 internal UART
UARTconf	equ 10h		; UART configuration reg
...
; fixed data tables for 2-drive in RAM
...
;	dw dirbf,dpblk3
;	dw chk03,all03
; disk parameter header for disk 04, RAM disk
;	dw 0,0
;	dw 0,0
;	dw dirbf,dpblk4
;	dw chk04,all04
...
dpblk0:
; drive A: contains cpm2.2 system files.  
; drive A starts from 0x10000
; 16K between the end of drive A and beginning of drive B is CPM2.2 executable & logical page 0
; Make BLS 2048 because cpmtools can't handle BLS of 1024 correctly

	dw 1024		;sectors per track (SPT)
	db 4		; block shift factor (BSH)
	db 15		;block mask (BLM)
	db 1		;null mask (EXM)
	dw 87		;disk size-1 (DSM), 176K bytes		
	dw 63		;directory max (DRM)	
	db 080h		;alloc 0 (AL0)
	db 0		;alloc 1 (AL1)		
	dw 0		;check size (CKS)
...
dpblk1:
; drive B: contains cpm2.2 system files.  The very last 4K of drive A: is 
;  4K physical page that maps to logical page 0
; track is 64K or 512 sectors.  drive B: starts from memory location 0x40000, no offset
; make BLS 2048 because cpmtools can't handle BLS of 1024 correctly
...
;	call MMUPage	; initialize MMU
;	call initMMU	; 16 system page descriptor are valid and point to first 64K 
...
;  data from LA9 thru LA20.  Note LA9 is CPM sector 37 (sector = 1+ (LA*4)) <-- not true
;  Use sector 36 as LA9 <-- not sure why
...
; CP/M executable is stored in 0x3D000-0x3FFFF.  Copy that to 0xDC00-0xFFFF
; 0x3D000 is effectively disk 0, track 1, sector 0x1A0

	ld b,0		; (track+1) is always 0
	ld c,1		; (track) is 1
	call SETTRK
	ld b,nsects	;reg B counts # of sectors to ...
	ld de,1a0h	; sector 0x1A0 is start of CP/M2.2 executable
; note that we begin by reading track 1 sector 0x1A0, the beginning of CPM
...
;;;  track+1 denotes disk number for disk bigger than 32meg
	...
	ld a,c
	ld (diskno),a
	cp 2		;must be between 0 and 2
	ret nc		;no carry if 2,3,...
; disk number is in the proper range
; compute proper disk parameter header address
	ld a,(diskno)
	ld l,a		;l=disk number 0,1
...
;so we will allow space to set up read command...
	...
	push de
	push bc
	ld c,4		; assuming drive B (RAMdisk) that starts at 0x40000 in RAM
	ld a,(diskno)	; check if RAMdisk
	cp 1
	jp z,RAMDISKr
	ld c,1		; not RAMdisk, so it must be drive A that starts at 0x10000 in RAM
; read RAM disk
; starting address in RAM is 0x40000
; starting address for drive A is 0x10000
;  calculate offset from the base of the page descriptor value
; each track is 1024 sectors of 128 byte/sec = 0x20000
;   shift (track) left by one + shift (MSB sector) right by one
;   shift (LSB sector) right 1bit with carry bit from MSB sector shift

;
;z calculate absolute address from track/sector values
;z
RAMDISKr:
	ld a,(track)	; offset= track*0x20000
	add a,a		; shift left by one
	ld hl,(sector)	; get sector value
	srl h
	rr l		; reg L is A[15..8] of address offset
	add a,h		; reg A is A[23..16] of address offset
;	add a,04h		; the RAMdisk starts from 0x040000
	add a,c		; reg C contains drive offset, 1 for drive A, 4 for drive B
	ld h,a		; HL now has valid A[23..8] into RAM disk
	push hl		;z save for later use
;z use DMA instead of MMU
	call DMAPage
...
;	outw (c),hl	; write DMA master control reg
	...
	ld c,DMA2cnt	; setup count of 128 byte
	ld hl,128
;	outw (c),hl	; write DMA2 count reg
	db 0edh,0bfh	; op code for OUTW (C),HL
	ld c,DMA2srcH	; source is RAMdisk
	pop hl		; get the calcuated RAM disk physical address
	ld a,0fh		; force lowest nibble of DMA2srcH to 0xF
	or l
	ld l,a		; low nibble of L is now 0xF
;	outw (c),hl	; write DMA2 source high reg
	db 0edh,0bfh	; op code for OUTW (C),HL
	xor a		; clear reg A
	ld l,a		; clear the lower half of HL reg
	ld a,(sector)	; calculate address offset within the 4K page
	and 1fh		; only interested in lowest 5 bits, the number of 128-byte sector
	srl a
	rr l		; A+L is now the address offset within the 4K page
	or 0f0h		; force high nibble to 0xF
	ld h,a		; HL now point the address [A11..A0] in RAM disk	
	ld c,DMA2srcL	; A11..A0 of source address
;	outw (c),hl	; write DMA2 source low reg
	db 0edh,0bfh	; op code for OUTW (C),HL
	ld c,DMA2dstH	; destination is what's in dmaad
	ld de,(dmaad)	; get the dmaad in reg de
;;mmu mark the code added when MMU is turned on by ZZ80Mon
;;mmu page 0 is mapped to 0x3C000, DMA moves to physical address, so
;;mmu test for dmaad in 0x0-0xFFF.  If dmaad is in page 0, add 0x3C000 offset to it
	ld a,d		;;mmu
	and 0f0h		;;mmu if highest nibble of dmaad is zero, add offset
	jp nz,notpage0	;;mmu
	ld l,0cfh		;;mmu if page 0, add 0x3C000 offset
	ld h,03h		;;mmu
	jp rdma2hi	;;mmu
notpage0:
	ld h,0		; clear h register
	ld a,0fh		; force lowest nibble of DMA2dstH to 0xF
	or d
	ld l,a		; A23..A16 is zero, A15..A12 is most significant nibble of dmaad
rdma2hi:		
;	outw (c),hl	; write DMA2 destination high reg
	db 0edh,0bfh	; op code for OUTW (C),HL
	ld c,DMA2dstL	;
	ld a,0f0h		; force high nibble of DMA2dstL to 0xFF
	or d
	ld h,a		; move the lower 12 bits of address to destination
	ld l,e
;	outw (c),hl	; write DMA2 destination low reg
	db 0edh,0bfh	; op code for OUTW (C),HL	
	ld hl,8080h	; enable DMA2, burst, byte size, flowthrough, no interrupt
;			;  incrementing memory for source & destination
	ld c,DMA2td	; setup DMA2 transaction descriptor reg
;	outw (c),hl	; write DMA2 transaction description reg
	db 0edh,0bfh	; op code for OUTW (C),HL
;  DMA should start now
	...
	nop
	nop
	call UARTPage	; return page i/o reg to default value
	
	pop bc		;restore reg
	pop de
	pop hl
	ld a,0		; return a OK status
	ret

; write to RAMdisk
; starting address in RAM disk is 0x40000
; starting address for drive A is 0x10000
WRITE:
	push hl		; save reg
	push de
...
;	call setupRD	; calculate the absolute memory address
;*********************setup RAM Disk ****************************
...
;	outw (c),hl	; write DMA3 count reg
	db 0edh,0bfh	; op code for OUTW (C),HL
	ld c,DMA3dstH	; destination is RAMdisk
	pop hl		; get the calcuated RAM disk physical address
...
;	outw (c),hl	; write DMA3 destination high reg
	...
	or 0f0h		; force high nibble to 0xF
	ld h,a		; HL now point the address [A11..A0] in RAM disk	
	ld c,DMA3dstL	; A11..A0 of destination address
;	outw (c),hl	; write DMA3 source low reg
	db 0edh,0bfh	; op code for OUTW (C),HL
	ld c,DMA3srcH	; source is what's in dmaad
	ld de,(dmaad)	; get the dmaad in reg de
;;mmu mark the code added when MMU is turned on by ZZ80Mon
;;mmu page 0 is mapped to 0x3C000, DMA moves to physical address, so
;;mmu test for dmaad in 0x0-0xFFF.  If dmaad is in page 0, add 0x3C000 offset to it
...
;	outw (c),hl	; write DMA3 source high reg
	db 0edh,0bfh	; op code for OUTW (C),HL
	ld c,DMA3srcL	;
	ld a,0f0h		; force high nibble of DMA3srcL to 0xFF
	or d
	ld h,a		; move the lower 12 bits of address to source reg
	ld l,e
;	outw (c),hl	; write DMA3 source low reg
	db 0edh,0bfh	; op code for OUTW (C),HL	
...
;	push bc
;	ld a,0ah		; V/WP/C/M bits are 1/0/1/0
;	ld hl,MMUtbl	;   valid, not write protect, cacheable
;initMMUp
;	ld (hl),a		; store page descriptor value to table
;	inc hl		; MSB of page descriptor
...
;	ldctl (c),hl	; write to I/O page register
	...
	pop bc
	ret
;;; initialize Z280 page i/o reg to point to MMU * DMA
MMUPage:
DMAPage:
	push bc		; save reg
	push hl
...
signon:	
	...
	db "CP/M 2.2 for ZZ80RC", 13,10
        	db "8/12/18 v1.1",13,10,0 
...
;all00: 	ds 31		;allocation vector 0-3
;all01: 	ds 31
;all02:	ds 31
;all03:	ds 31
all00:	ds 32
