...
; protoCF with base address at 0x10
...
;;8 CP/M executable is stored in 0x3D000-0x3FFFF.  Copy that to 0xDC00-0xFFFF
;;8 0x3D000 is effectively disk 0, track 1, sector 0x1A0

	ld b,0		;;8 (track+1) is always 0
	ld c,1		;;8 (track) is 1
	call SETTRK	;;8
	ld b,nsects	;;8 reg B counts # of sectors to ...
	ld de,1a0h	;;8 sector 0x1A0 is start of CP/M2.2 executable
...
;so we will allow space to set up read command...
	...
	push de
	push bc
	ld c,1		;;8 assuming drive A (RAMdisk that starts at 0x10000 in RAM)
	ld a,(diskno)	; check if RAMdisk	
	cp 0		;;8 check RAMdisk A
	jp z,RAMDISKr
	ld c,4		;;8 not drive A, assume it is drive B (RAMdisk starts at 0x40000)
	cp 1		;;8 check RAMdisk B
	jp z,RAMDISKr
;;8	push hl		;save reg
;;8	push de
;;8	push bc
...
;	nop
; read RAM disk
; starting address in RAM is 0x700000
RAMDISKr:
;;8	push hl		; save reg
;;8	push de
;;8	push bc
...
; write to RAMdisk
; starting address in RAM is 0x080000
RAMDISKw:
	ld a,(track)	; offset= track*0x20000
	add a,a		; shift left by one
	ld hl,(sector)	; get sector value
...
;  DMA should start now
	...
	ret
;;8 write to RAMdisk
;;8 starting address in RAM disk is 0x40000
;;8 starting address for drive A is 0x10000
WRITE:
...
signon:	
	...
	db "CP/M 2.2 for ZZ80RC", 13,10
        	db "10/2/18 external CF disk",13,10,0 
...
all00:	ds 32		;;8 small RAM disk
all01:	ds 32		;;8 small RAM disk
all02:	ds 256
all03:	ds 256
all04:	ds 256
